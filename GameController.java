...
public class GameController
{
   ...
   Factory st = new Factory(1,5,2,10,new Color(.3,.3,.3,1)); //2
   Factory mt = new Factory(1,3,4,20,new Color(.6,.6,.3,1)); //4
   Factory art = new Factory(3,2,4,8,new Color(0,.6,.6,1));  //6 
   Factory rk = new Factory(5,1,6,5,new Color(.6,0,.6,1));  //8 
      ...
      for(int i=0;i<templatemap.length;i++)
      {
         for(int j=0;j<templatemap[0].length;j++)
         {
            ...
            if(spot != 0 && spot != -1)
            {
               ...
               if(unit == 3) //art
               {
                  u = art.createUnit(j,i,owner,owner == 0? playerZero : playerOne, Unit.Type.ART);
               }
               if(unit == 4) //rk
               {
